...
function thanhtoan(btnThanhToan) {
    if (btnThanhToan.value == 0) {
        alert('Giỏ hàng của bạn đang rỗng, hãy chọn loại xe để thanh toán!')
    }
    else {
...
// trang thanh toán sẽ được cập nhật liên tục mỗi khi thêm hoặc xóa loại xe
window.onload = function () {
    var tongtien = 0;
    var itemList = JSON.parse(localStorage.getItem('Names'));
    //thêm từng loại xe vào trong trang từ localStorage
    if (itemList != null) {
        ...
        for (var i = 0; i < itemList.length; i++) {
            ...
            li.innerHTML = `<div class="item-box">
                            ...
                            <div class="item-detail">
                                ...
                                <p class="item-money">${Number(item.price).toLocaleString("vi")} VNĐ/Ngày</p>
                            </div>
                        </div> `
            ul.appendChild(li);
        ...
        h2.innerHTML = `Tổng số tiền: ${Number(tongtien).toLocaleString("vi")} VNĐ/Ngày`;
